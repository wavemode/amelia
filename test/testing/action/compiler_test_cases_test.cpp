...
TEST_CASE("collect_test_cases") {
  ...
  collect_test_cases(filesystem_walker, file_loader, collection, "test/testing/action/collect");

  REQUIRE(collection.test_cases.size() == 2);
  REQUIRE(
      collection.test_cases[0].filename ==
      "test/testing/action/collect/collect1/collect2/test2.am"
  );
  REQUIRE(collection.test_cases[0].expected_output == "testing2\n");
  REQUIRE(collection.test_cases[1].filename == "test/testing/action/collect/collect1/test1.am");
...
TEST_CASE("execute_test_case") {
  ...
  String test_case_path = "test/testing/action/execute/example1.am";
  String file_contents = "x = y\n";
  file_writer.write_file(test_case_path, file_contents);

  CompilerTestCaseCollection collection;
  collect_test_cases(filesystem_walker, file_loader, collection, "test/testing/action/execute");
  ...
  collect_test_cases(
      filesystem_walker, file_loader, collection2, "test/testing/action/execute"
  );
