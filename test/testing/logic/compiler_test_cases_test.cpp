...
TEST_CASE("collect_test_cases") {
  ...
  collect_test_cases(filesystem_walker, file_loader, collection, "test/testing/logic/collect");

  REQUIRE(collection.test_cases.size() == 2);
  REQUIRE(
      collection.test_cases[0].filename ==
      "test/testing/logic/collect/collect1/collect2/test2.am"
  );
...
TEST_CASE("execute_test_case") {
  ...
  CompilerTestCaseCollection collection;
  collect_test_cases(filesystem_walker, file_loader, collection, "test/testing/logic/execute");
  ...
  collect_test_cases(
      filesystem_walker, file_loader, collection2, "test/testing/logic/execute"
  );
