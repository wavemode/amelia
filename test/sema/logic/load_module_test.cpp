...
TEST_CASE("target module does not exist") {
  ModuleLoaderContext ctx{Set<String>({"test/sema/logic/empty"})};
  FileLoader file_loader;
  SemaResult sema_result;
...
TEST_CASE("imported module does not exist") {
  ModuleLoaderContext ctx{Set<String>({"test/sema/logic/a_alone"})};
  FileLoader file_loader;
  SemaResult sema_result;
...
TEST_CASE("imported module has the same name in different directories with same contents") {
  ModuleLoaderContext ctx{Set<String>(
      {"test/sema/logic/c_alone_import_a",
...
TEST_CASE("submodule has the same name in different directories with different contents") {
  ModuleLoaderContext ctx{
      Set<String>({"test/sema/logic/basic_sub", "test/sema/logic/basic_sub_diff"})
  };
  FileLoader file_loader;
  SemaResult sema_result;
  load_module(file_loader, sema_result, "a", ctx);
...
TEST_CASE("single file declares two submodules with the same name") {
  ModuleLoaderContext ctx{Set<String>({"test/sema/logic/duplicate_submodule"})};
  FileLoader file_loader;
  SemaResult sema_result;
  CHECK_THROWS_AS(load_module(file_loader, sema_result, "a", ctx), SourceLocationError);
}
...
TEST_CASE("a -> b") {
  ModuleLoaderContext ctx{Set<String>({"test/sema/logic/basic"})};
  FileLoader file_loader;
  SemaResult sema_result;
  load_module(file_loader, sema_result, "a", ctx);
  REQUIRE(sema_result.module_ids.size() == 2);
...
TEST_CASE("a -> b/c") {
  ModuleLoaderContext ctx{Set<String>({"test/sema/logic/subdir"})};
  FileLoader file_loader;
  SemaResult sema_result;
  load_module(file_loader, sema_result, "a", ctx);
  REQUIRE(sema_result.module_ids.size() == 2);
...
TEST_CASE("b/c -> a") {
  ModuleLoaderContext ctx{Set<String>({"test/sema/logic/subdir_rev"})};
  FileLoader file_loader;
  SemaResult sema_result;
...
TEST_CASE("a -> b::c") {
  ModuleLoaderContext ctx{Set<String>({"test/sema/logic/basic_sub"})};
  FileLoader file_loader;
  SemaResult sema_result;
  load_module(file_loader, sema_result, "a", ctx);
...
TEST_CASE("b::c -> a") {
  ModuleLoaderContext ctx{Set<String>({"test/sema/logic/sub_rev"})};
  FileLoader file_loader;
  SemaResult sema_result;
  load_module(file_loader, sema_result, "b::c", ctx);
...
TEST_CASE("2 circular imports") {
  ModuleLoaderContext ctx{Set<String>({"test/sema/logic/two_circular"})};
  FileLoader file_loader;
  SemaResult sema_result;
  load_module(file_loader, sema_result, "a", ctx);
...
TEST_CASE("3 circular imports") {
  ModuleLoaderContext ctx{Set<String>({"test/sema/logic/three_circular"})};
  FileLoader file_loader;
  SemaResult sema_result;
  load_module(file_loader, sema_result, "a", ctx);
...
TEST_CASE("a -> b::c -> a") {
  ModuleLoaderContext ctx{Set<String>({"test/sema/logic/sub_circular"})};
  FileLoader file_loader;
  SemaResult sema_result;
  load_module(file_loader, sema_result, "a", ctx);
...
TEST_CASE("a -> (b <-> c) -> d") {
  ModuleLoaderContext ctx{Set<String>({"test/sema/logic/circle_in_chain"})};
  FileLoader file_loader;
  SemaResult sema_result;
  load_module(file_loader, sema_result, "a", ctx);
...
TEST_CASE("a -> (b <-> c <-> d) -> e") {
  ModuleLoaderContext ctx{Set<String>({"test/sema/logic/three_circle_in_chain"})};
  FileLoader file_loader;
  SemaResult sema_result;
  load_module(file_loader, sema_result, "a", ctx);
...
TEST_CASE("(a <-> b) + (a <-> c)") {
  ModuleLoaderContext ctx{Set<String>({"test/sema/logic/overlapping"})};
  FileLoader file_loader;
  SemaResult sema_result;
  load_module(file_loader, sema_result, "c", ctx);
