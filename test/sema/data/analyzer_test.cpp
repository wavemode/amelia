...
TEST_CASE("test suite") {
  ...
  collect_test_cases(
      filesystem_walker, file_loader, console_printer, env_reader, collection, "test_cases/sema"
  );
  ...
  for (const auto &test_case : collection.test_cases) {
    ...
    CHECK(execute_test_case(
        sema_test_case_runner, file_writer, console_printer, outcome, test_case, execution_context
    ));
  }
