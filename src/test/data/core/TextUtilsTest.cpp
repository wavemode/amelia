...
TEST_CASE("slice") {
  ...
  SUBCASE("slice with length greater than remaining") {
    Text input = "hello world";
    CharIterator it = input.begin().plus(6);
    CHECK_THROWS_AS(TextUtils::slice(input, it, 10), std::out_of_range);
  }
}
...
TEST_CASE("slice_bytes") {
  ...
  SUBCASE("slice_bytes with length greater than remaining") {
    Text input = "hello world";
    int64_t byte_index = 6;
    CHECK_THROWS_AS(TextUtils::slice_bytes(input, byte_index, 10), std::out_of_range);
  }
}
...
TEST_CASE("substr") {
  ...
  SUBCASE("substr with end before start") {
    ...
    CHECK_THROWS_AS(TextUtils::substr(input, start, end), std::out_of_range);
  }
}
...
TEST_CASE("substr_bytes") {
  ...
  SUBCASE("substr_bytes with end before start") {
    ...
    CHECK_THROWS_AS(
        TextUtils::substr_bytes(input, start_byte_index, end_byte_index), std::out_of_range
    );
  }
}
