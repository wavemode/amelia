...
TEST_CASE("imported module is in second directory in path") {
  ModuleLoaderContext ctx{
      Set<String>({"src/test/action/sema/a_alone", "src/test/action/sema/b_alone"})
  };
  FileLoader file_loader;
  SemaResult sema_result;
  load_module(file_loader, sema_result, "a", ctx);
...
TEST_CASE("target module has the same name in different directories with same contents") {
  ModuleLoaderContext ctx{Set<String>(
      {"src/test/action/sema/a_alone",
       "src/test/action/sema/a_alone_dup",
       "src/test/action/sema/b_alone"}
  )};
  FileLoader file_loader;
  SemaResult sema_result;
  load_module(file_loader, sema_result, "a", ctx);
...
TEST_CASE("imported module has the same name in different directories with same contents") {
  ModuleLoaderContext ctx{Set<String>(
      {"src/test/action/sema/c_alone_import_a",
       "src/test/action/sema/a_alone",
       "src/test/action/sema/a_alone_dup",
       "src/test/action/sema/b_alone"}
  )};
...
TEST_CASE("submodule has the same name in different directories with same contents") {
  ModuleLoaderContext ctx{
      Set<String>({"src/test/action/sema/basic_sub", "src/test/action/sema/basic_sub_dup"})
  };
  FileLoader file_loader;
  SemaResult sema_result;
  load_module(file_loader, sema_result, "a", ctx);
...
TEST_CASE("target module has the same name in different directories with different contents") {
  ModuleLoaderContext ctx{Set<String>(
      {"src/test/action/sema/a_alone",
       "src/test/action/sema/a_alone_diff",
       "src/test/action/sema/b_alone"}
  )};
  FileLoader file_loader;
  SemaResult sema_result;
...
TEST_CASE("submodule has the same name in different directories with different contents") {
  ModuleLoaderContext ctx{
      Set<String>({"src/test/action/sema/basic_sub", "src/test/action/sema/basic_sub_diff"})
  };
  FileLoader file_loader;
  SemaResult sema_result;
  load_module(file_loader, sema_result, "a", ctx);
...
TEST_CASE("single file declares two submodules with the same name") {
  ModuleLoaderContext ctx{Set<String>({"src/test/action/sema/duplicate_submodule"})};
  FileLoader file_loader;
  SemaResult sema_result;
  CHECK_THROWS_AS(load_module(file_loader, sema_result, "a", ctx), SourceLocationError);
}
...
TEST_CASE("a -> (b <-> c) -> d") {
  ...
  REQUIRE(sema_result.module_meta[module_d_id].imported_ids.size() == 0);

  REQUIRE(sema_result.module_meta[module_d_id].imported_by_ids.size() == 1);
...
TEST_CASE("a -> (b <-> c <-> d) -> e") {
  ...
  REQUIRE(sema_result.module_meta[module_e_id].imported_ids.size() == 0);

  REQUIRE(sema_result.module_meta[module_e_id].imported_by_ids.size() == 1);
  REQUIRE(sema_result.module_meta[module_e_id].imported_by_ids.has(module_d_id));
