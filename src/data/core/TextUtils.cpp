...
void TextUtils::split(List<Text> &output, Text input, Text delimiter, int64_t max_splits) {
  if (delimiter.size() == 0) {
...
void TextUtils::split(List<String> &output, Text input, Text delimiter, int64_t max_splits) {
...
void TextUtils::find_all(List<CharIterator> &output, Text input, Text substring) {
  CharIterator it = input.begin();
  while (!it.at_end()) {
...
void TextUtils::find_all_bytes(List<int64_t> &output, Text input, Text substring) {
  CharIterator it = input.begin();
  while (!it.at_end()) {
    CharIterator found = find_after(input, substring, it);
    if (found.at_end()) {
...
void TextUtils::find_all_chars(List<CharIterator> &output, Text input, uint32_t code_point) {
  CharIterator it = input.begin();
  while (!it.at_end()) {
...
void TextUtils::find_all_char_bytes(List<int64_t> &output, Text input, uint32_t code_point) {
  CharIterator it = input.begin();
  while (!it.at_end()) {
    CharIterator found = find_char_after(input, code_point, it);
    if (found.at_end()) {
...
Text TextUtils::slice_bytes(Text input, size_t index_start, size_t byte_length) {
  if (index_start + byte_length > input.size()) {
    throw std::out_of_range("Byte slice is out of range");
  }
...
Text TextUtils::substr(Text input, CharIterator char_start, CharIterator char_end) {
  ...
  if (start_offset > end_offset || end_offset > input.size()) {
    throw std::out_of_range("Character slice is out of range");
  }
